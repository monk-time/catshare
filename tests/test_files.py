...
def test_backend_dokerfile_content(
    ...
    with (path / dockerfile_name).open(encoding='utf-8', errors='ignore') as f:
...
def test_workflow_file(base_dir: Path, workflow_file_name: str):
    ...
    assert path_to_file.is_file(), (
        ...
        f'`{workflow_file_name}`, в котором описан workflow для CatShare.'
    )
    with path_to_file.open(encoding='utf-8', errors='ignore') as stream:
        try:
